...
export function DashboardPage() {
  ...
  const cards = summary
    ? [
        ...
        ["Endpoints", summary.counts.total],
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Link className="rounded-xl bg-emerald-400 px-5 py-3 font-bold text-slate-950 hover:bg-emerald-300" to="/projects/new">Add project</Link>
      </div>
        ...
        <>
          ...
          <section className="mt-8 rounded-2xl border border-slate-800 bg-slate-900 p-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <Link className="text-sm font-semibold text-emerald-400" to="/projects">All projects</Link>
            </div>
            {summary.recentFailures.length === 0 ? (
              ...
              <div className="mt-5 overflow-x-auto">
                <table className="w-full min-w-[620px] text-left text-sm">
                  <thead className="text-xs uppercase text-slate-500"><tr><th className="pb-3">Project / endpoint</th><th className="pb-3">Failure</th><th className="pb-3">Response</th><th className="pb-3">Time</th></tr></thead>
                  <tbody className="divide-y divide-slate-800">
                    {summary.recentFailures.map((failure) => (
                      <tr key={failure.id}>
                        <td className="py-4"><Link className="font-bold hover:text-emerald-400" to={`/endpoints/${failure.monitorId}`}>{failure.endpointName}</Link><p className="mt-1 text-xs text-slate-500">{failure.project.name}</p></td>
